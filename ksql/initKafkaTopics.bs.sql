...
-- Info zur verwendeten ksqldb-Version: http://localhost:8088/info
-- {"KsqlServerInfo":{"version":"0.9.0","kafkaClusterId":"vpxcigCOQI6ig7_8Ei7OsA","ksqlServiceId":"kafka-demo"}}
...
select * from s_rawGames emit changes limit 10;



CREATE STREAM s_rawGamesTS WITH (
    TIMESTAMP='ts',
    timestamp_format='yyyy.MM.dd''T''HH:mm:ss.SSS'
) AS
Select * from s_rawGames emit changes;
...
select *, rowtime, TIMESTAMPTOSTRING(ROWTIME, 'yyyy-MM-dd HH:mm:ss.SSS') AS rowtime_formatted from s_rawGamesTS emit changes limit 10;
drop Stream s_rawGames;
drop Stream s_rawGamesTS ;
DROP Stream IF EXISTS s_rawGamesTS delete topic;
TERMINATE CSAS_S_RAWGAMESTS_37;
